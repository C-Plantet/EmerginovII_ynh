...
testCM("coordsCharBidi", function (cm) {
  addDoc(cm, 35, 70); // Put an rtl character into each line to trigger the bidi code path in coordsChar
...
testCM("bookmarkInsertLeft", function (cm) {
  ...
  cm.replaceRange("", Pos(0, 1), Pos(0, 2)); // Verify that deleting next to bookmarks doesn't kill them
...
testCM("wrappingAndResizing", function (cm) {
  ...
  is(wrap.offsetHeight > h0, "wrapping happens"); // Now add a max-height and, in a document consisting of
...
}, null, ie_lt9); // Verify that updating a line clears its bidi ordering
...
testCM("lineWidgetChanged", function (cm) {
  ...
  function w() {
    var node = document.createElement("div"); // we use these children with just under half width of the line to check measurements are made with correct width
...
testCM("lineWidgetIssue5486", function (cm) {
  ...
  var measure_1 = Math.round(cm.charCoords({
    line: 2,
    ch: 0
  }).top); // resize lineWidget, height + 50 px

  el.style.height = '100px';
  el.textContent += "\nlineWidget size changed.\nTry moving cursor to line 3?";
  lineWidget.changed(); // re-measure 3rd line position

  var measure_2 = Math.round(cm.charCoords({
    line: 2,
    ch: 0
  }).top);
  eq(measure_2, measure_1 + 50); // (extra test)
  //
...
testCM("atomicMarker", function (cm) {
  ...
  cm.execCommand("goCharLeft");
  eqCursorPos(cm.getCursor(), Pos(0, 1));
  m.clear(); // Can't cursor to the left of a marker when inclusiveLeft=true

  m = atom(0, 0, 0, 5, true);
  eqCursorPos(cm.getCursor(), Pos(0, 5), "pushed out");
  cm.execCommand("goCharLeft");
  eqCursorPos(cm.getCursor(), Pos(0, 5));
  m.clear(); // Can't cursor to the left of a marker when inclusiveLeft=false and selectLeft=false
  ...
  cm.execCommand("goCharLeft");
  eqCursorPos(cm.getCursor(), Pos(0, 5));
  m.clear(); // Can cursor to the left of a marker when inclusiveLeft=false and selectLeft=True
  ...
  cm.execCommand("goCharLeft");
  eqCursorPos(cm.getCursor(), Pos(0, 0));
  m.clear(); // Can't cursor to the right of a marker when inclusiveRight=true
  ...
  m.clear(); // Can't cursor to the right of a marker when inclusiveRight=false and selectRight=false
  ...
  cm.execCommand("goCharRight");
  eqCursorPos(cm.getCursor(), Pos(0, 6));
  m.clear(); // Can cursor to the right of a marker when inclusiveRight=false and selectRight=True
  ...
  cm.execCommand("goCharRight");
  eqCursorPos(cm.getCursor(), Pos(0, 5));
  m.clear(); // Can't cursor to the right of a multiline marker when inclusiveRight=true
  ...
  m.clear(); // Cursor jumps across a multiline atomic marker,
  ...
  m.clear();
  addDoc(cm, 10, 10); // Delete before an atomic marker deletes the entire marker
...
;
var bidiTests = []; // We don't correctly implement L1 UBA
...
bidiTests.push("քմѧǮßپüŢҍҞўڳ\nӧ"); //bidiTests.push("Count ١ ٢ ٣ ٤");
...
function testCoordsWrappedBidi(str) {
  testCM("coords_wrapped_bidi_" + str, function (cm) {
    cm.getScrollerElement().style.fontFamily = "monospace";
    makeItWrapAfter(cm, Pos(0, 5)); // Make sure we are at the visual beginning of the first line
